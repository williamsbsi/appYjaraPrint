...
export class Escpos {
    ...
    init() {
        ...
        this.buffer += CMD.HARDWARE.HW_INIT;
        return this;
    }

    boldOn() {
        this.buffer += CMD.TEXT_FORMAT.TXT_BOLD_ON;
        return this;
    }

    boldOff() {
        this.buffer += CMD.TEXT_FORMAT.TXT_BOLD_OFF;
        return this;
    }

    marginBottom(size) {
        this.buffer += CMD.MARGINS.BOTTOM;
        this.buffer += size;
        return this;
    }

    marginLeft(size) {
        this.buffer += CMD.MARGINS.LEFT;
        this.buffer += size;
        return this;
    }
    ...
    lineSpace(n = null) {
        if (n === null) {
            this.buffer += CMD.LINE_SPACING.LS_DEFAULT;
        } else {
            this.buffer += CMD.LINE_SPACING.LS_SET;
            this.buffer += n;
        }
        return this;
    }
    ...
    barcode(code, type = 'CODE128', width = 3, height = 100, position = 'BTH', font = 'B') {
        ...
        const codeBytes = code.split('').map(s => s.charCodeAt(0));
